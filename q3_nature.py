...
class NatureQN(Linear):
    ...
    def get_q_values_op(self, state, scope, reuse=False):
        ...
        with tf.variable_scope(scope, reuse=reuse):
            ...
            out = layers.conv2d(inputs=out, num_outputs=16, kernel_size=8, stride=4)
            out = layers.conv2d(inputs=out, num_outputs=32, kernel_size=4, stride=2)
            out = layers.flatten(inputs=out)
            out = layers.fully_connected(inputs=out, num_outputs=256)
