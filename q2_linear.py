...
class Linear(DQN):
    ...
    def add_loss_op(self, q, target_q):
        ...
        self.loss = tf.reduce_mean(tf.where(tf.less(residual, delta), 0.5 * tf.square(residual), delta * residual - 0.5 * tf.square(delta)))
    ...
    def add_optimizer_op(self, scope):
        ...
        gs, vs = zip(*optimizer.compute_gradients(self.loss, var_list=tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=scope)))
